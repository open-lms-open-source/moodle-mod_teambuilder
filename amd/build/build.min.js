...
for(i=1;i<m.length;i++)t=m[i],lt=m[q],t.length<lt.length?(q=i,u=[]):t.length==lt.length&&u.push(i);u.push(q);
...
for(s in a)L(s,criterion)&&b.inArray(s,candidates)==-1&&candidates.push(s)}return candidates},L=function(c,d){// Student responses.
if(sr=responses[c],sr===!1)return!1;var e;// Return value.
if("any"==d.oper){e=!1;for(a in d.answers)if(ans=parseInt(d.answers[a]),b.inArray(ans,sr)!=-1){e=!0;break}}else if("all"==d.oper){e=!0;for(a in d.answers)if(ans=parseInt(d.answers[a]),b.inArray(ans,sr)==-1){e=!1;break}}else if("none"==d.oper){e=!0;for(a in d.answers)ans=parseInt(d.answers[a]),b.inArray(ans,sr)!=-1&&(e=!1)}if(e&&d.subcriteria)for(i=0;i<d.subcriteria.length&&(sc=d.subcriteria[i],e=L(c,sc),0!=e);i++);return e},M=function(){G();var a=[];for(b("#unassigned .student").each(function(){rslt=/student-(\d+)/.exec(this.id),a.push(rslt[1])}),a=P(a);a.length>0;){
// Get the team(s) with the lowest numbers.
var c=0,d=[];
// Skip the 0th team since otherwise we compare it to itself.
